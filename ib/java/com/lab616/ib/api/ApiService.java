...
public class ApiService extends AbstractEventWatcher 
  ...
  private final ApiClient.Factory factory;
  private final Map<String, ApiClient> apiClients = Maps.newHashMap();
  
  @Inject
  public ApiService(ApiClient.Factory factory) {
      ...
      if ("start".equals(event.getMethod())) {
        ...
        synchronized (apiClients) {
          if (!apiClients.containsKey(name)) {
            ...
            ApiClient client = this.factory.create(name, id);
            client.connect();
      ...
      if ("stop".equals(event.getMethod())) {
        ...
        synchronized (apiClients) {
          if (apiClients.containsKey(name)) {
            ...
            ApiClient client = apiClients.get(name);
            client.disconnect();
