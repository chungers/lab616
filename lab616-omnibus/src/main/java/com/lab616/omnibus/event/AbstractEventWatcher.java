...
public abstract class AbstractEventWatcher {
  ...
  private CountDownLatch addedToEngineLatch = new CountDownLatch(1);
  private AtomicBoolean addedToEngine = new AtomicBoolean(false);

  public boolean isReady(long... timeout) {
    try {
      if (timeout.length > 0) {
        addedToEngineLatch.await(timeout[0], TimeUnit.MILLISECONDS);
      } else {
        addedToEngineLatch.await();
      }
    } catch (Exception e) {
      // Interrupted.
    }
    return addedToEngine.get();
  }

  void addedToEngine() {
    addedToEngine.set(true);
    addedToEngineLatch.countDown();
  }
