...
/**
 * For splitting a stream into multiple streams.  See Esper manual Section 4.18.
 * For example, the EPL statement generated is:
 * on OrderEvent 
 ...
 *
 * Note that for a 'on...' clause, there are multiple insert/where clauses, each
 * completes a specification of stream with the name given.
 * 
 * @author david
 */
public class StreamSplitter<T> {
  ...
  public class Direct {
    ...
    public class SubscriberClause {
      ...
      public <S extends Subscriber<?>> StreamSplitter<T> to(S subscriber) {
        sub = subscriber;
        return StreamSplitter.this;
      }
    }
