...
public class AvroDataFile {
  ...
  public AvroDataFile(String dir, String root, int part) {
  	this(dir, root);
  	this.part = part;
  }
  ...
  public Reader getReader() throws IOException {
    DateTime today = new DateTime().withMillisOfDay(0);
    return new Reader(formatName(today, part));
  }
  
  private String formatName(DateTime today, int part) {
  	return (part == 0) ?
  ...
  private String getFileName(DateTime today) {
    ...
    int tries = 0;
    do {
      String fn = formatName(today, tries);
      f = new File(fn);
      tries++;
    } while (f.exists());
    return f.getPath();
