...
public class TWSBlockingCallManager {
  ...
  private Map<Thread, FutureHolder<EventMessage>> futureData = Maps.newHashMap();
   ...
   */
  public void handleData(EventMessage event) {
    for (FutureHolder<EventMessage> future : futureData.values()) {
  ...
  public <V> V blockingCall(String method, 
      Function<EventMessage, V> trans, Runnable call) {
  ...
  public <V> V blockingCall(final String method, long timeout, TimeUnit unit,
      Function<EventMessage, V> trans, Runnable call, 
      final Predicate<EventMessage>... predicates) {
    ...
    return blockingCall(timeout, unit,
      ...
      public boolean apply(EventMessage event) {
        ...
        if (matched) {
          for (Predicate<EventMessage> p : predicates) {
            if (p.apply(event)) {
              return true;
            }
          }
  ...
  public <V> V blockingCall(long timeout, TimeUnit unit,
    ...
    FutureData<EventMessage, V> f = new FutureData<EventMessage, V>(
        this.executor,
  ...
  public <V> Iterable<V> blockingIterable(long timeout, TimeUnit unit,
      ...
      Function<EventMessage, V> trans, Runnable call) {
    // Create a future data.
    FutureIterable<EventMessage, V> f = new FutureIterable<EventMessage, V>(
        timeout, unit, predicate, trans);
    this.futureData.put(Thread.currentThread(), f);
