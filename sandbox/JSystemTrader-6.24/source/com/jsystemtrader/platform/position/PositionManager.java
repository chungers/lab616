...
public class PositionManager {
    ...
    private double grossProfit, grossLoss, peakTotalProfitAndLoss, maxDrawdown, kelly;
    private StandardDeviation profitableTradeStandardDeviation, unprofitableTradeStandardDeviation;
...
//    }
    ...
    public synchronized void update(OpenOrder openOrder) {
        ...
        else if(oldPosition==0.0 || (oldPosition<0&&position>0) || (oldPosition>0&&position<0))
        {
            avgFillPrice = orderAvgFillPrice;
        }
        // Case 3: avgFillprice is increasing or decreasing
        else if((oldPosition<0&&position<oldPosition) || (oldPosition>0&&position>oldPosition))
        {
            avgFillPrice = ( Math.abs(oldPosition)*avgFillPrice + tradeAmount ) / Math.abs(position);
        }
        ...
        {
            double wfaktor =(double) profitableTrades / (profitableTrades + unprofitableTrades);
            double rfaktor = (double)(grossProfit / profitableTrades)/ (Math.abs(grossLoss / unprofitableTrades));
        ...
        positionsHistory.add(new Position(openOrder.getDate(), position, avgFillPrice, orderAvgFillPrice));


        if ((Dispatcher.getMode() != Dispatcher.Mode.OPTIMIZATION)) {
            ReportRenderer renderer = eventReport.getRenderer();
            ...
            msg.append("Avg Fill Price: ").append(avgFillPrice).append(".").append(fieldBreak);
            msg.append(emphasisStart).append(" Position changed to: ").append(getPosition()).append(emphasisEnd).append(fieldBreak);
            msg.append("This trade P&L: ").append(nf4.format(profitAndLoss)).append(" points ").append(result).append(".").append(fieldBreak);
    ...
    public synchronized void update(PortfolioMirrorItem portfolioMirrorItem)
    {
    ...
    public void trade()
    {
        ...
        Dispatcher.getTrader().getAssistant().placeMarketOrder(contract, Math.abs(quantity), action, strategy);
