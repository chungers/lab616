...
public class StrategyOptimizerRunner extends Thread {
    ...
    public StrategyOptimizerRunner(OptimizerDialog optimizerDialog, String strategyName, StrategyParams strategyParams) throws ClassNotFoundException, NoSuchMethodException {
    ...
    private void saveToFile(Strategy strategy) throws IOException, JSystemTraderException {
        if (results.size() == 0) {
            return;
        }
    ...
    private void showProgress(long counter) {
    ...
    public void run() {
        try {
            ...
            if (iterations > MAX_ITERATIONS) {
                optimizerDialog.showMaxIterationsLimit(iterations, MAX_ITERATIONS);
                return;
            }
            ...
            int maxThreads = Integer.valueOf(jstProperties.get(OptimizerMaxThread));
            for (int thread = 0; thread < maxThreads; thread++) {
                new OptimizerWorker(strategyConstructor, tasks, results, minTrades, remainingTasks).start();
            }

            optimizerDialog.showProgress("Estimating remaining time...");
            long startTime = System.currentTimeMillis();
            timeEstimator = new ComputationalTimeEstimator(startTime, iterations);
