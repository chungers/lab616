...
public class StrategyRunner implements Runnable, ErrorListener {
    ...
    public void error(int errorCode, String errorMsg) {
        ...
        boolean hasQueryExpired = (errorCode == 320 && errorMsg.contains("Invalid Real-time Query"));
        ...
        if (hasQueryExpired || isRestoredWithDataLost) {
            eventReport.report(strategy.getName() + ": Re-subscribing to real time bars");
            getRealTimeBars();
        }
    ...
    public StrategyRunner(Strategy strategy) throws IOException, JSystemTraderException {
        isConnected = true;
    ...
    private void getRealTimeBars() {
        ...
        String priceType = strategy.getQuoteHistory().getIsForex() ? "MIDPOINT" : "TRADES";
        trader.getAssistant().getRealTimeBars(strategyID, contract, 5, priceType, false);
    }
    ...
    private void getHistoricalData() throws InterruptedException {
        ...
        String histRequestDuration = strategy.getBarSize().getHistRequestDuration();
        Contract contract = strategy.getContract();
        int strategyID = strategy.getId();
        String timeNow;
        ...
        /*
          ...
          multiple requests for the same ticker ending with the same time
          are issued, even if these requests are spaced out. Since it's
          possible that multiple strategies run at the same time and
          request the same historical data, we simply add a few seconds to
          the "end time" to get around this limitation. The strategy ID is
          a unique number, so it will guarantee that the end time is unique.
        */
        ...
        trader.getAssistant().getHistoricalData(strategyID, contract, timeNow, histRequestDuration, barSize, priceType, onlyRTHPriceBars, 2);
        synchronized (trader) {
    ...
    public void run() {
        try {
            ...
            if (!strategy.getTradingSchedule().isTimeToTrade()) {
                eventReport.report(strategy.getName() + ": Waiting for the start of the first trading interval.");
                strategy.getTradingSchedule().waitForTradingInterval();
                eventReport.report(strategy.getName() + ": Trading interval started.");
            }
    ...
    private void processEvent(QuoteHistoryEvent quoteHistoryEvent) {
        QuoteHistoryEvent.EventType eventType = quoteHistoryEvent.getType();

        if (eventType == NEW_BAR) {
        ...
        if (eventType == NEW_BAR && !positionManager.isOrderExecutionPending()) {
            strategy.update();
        }
