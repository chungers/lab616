...
/**
 * Uses OpenTick API to download historical price and volume data for
 * specified exchange and security. Saves the downloaded data in a file
 * as date- and time-stamped OHLCV price-volume bars.
 */
public class OTBackDataDownloader {
    ...
    public OTBackDataDownloader(OTBackDataDialog backDataDialog) throws OTLoginException {
        ...
        openTickClient.login(backDataDialog.getUserName(), backDataDialog.getPassword());
    }
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
                        ...
                        MessageDialog.showError(backDataDialog, ote.getMessage());
                    }
                }
            }).start();
        } else {
    ...
    public void getSecurities(String exchangeCode) throws OTException {
        eventReport.report("Requested list of OpenTick symbols for exchange " + exchangeCode);
        openTickClient.requestListSymbols(exchangeCode);
    }

    public void setSecurities(List<?> securities) {
        backDataDialog.setIndeterminateProgress("Processing securities for exchange " + backDataDialog.getFullExchange());
        ...
        String[] securitiesArray = allSecurities.toArray(new String[allSecurities.size()]);
    ...
    public void error(OTError error) {
        if (!openTickClient.isLoggedIn()) {
            backDataDialog.enableLogin();
        }
        isCancelled = true;
        backDataDialog.closeProgress();
        String msg = "Open Tick error " + error.getCode() + ": " + error.getDescription();
    ...
    private void downloadBars() {
        try {
            ...
            if (barSize >= 60) {
                requestID = openTickClient.requestHistData(entity, start, end, OTConstants.OT_HIST_OHLC_MINUTELY, barSize / 60);
            } else {
                tickAccumulator = new TickAccumulator(barSize);
                requestID = openTickClient.requestHistTicks(entity, start, end, OTConstants.OT_TICK_TYPE_TRADE);
            }
            String msg = "Requested OpenTick historical data for " + exchange + "/" + security + ", requestID: " + requestID;
            eventReport.report(msg);


            backDataDialog.setProgress(0, "Initializing...", "Downloading historical data for " + security);
            isCancelled = false;
            ...
            while (!responseCompleted && !isCancelled) {
                ...
                backDataDialog.setProgress((int) progress, progressMsg, "Downloading historical data for " + security);
            }
            ...
            if (!isCancelled) {
                ...
                writeDescriptior();
                writeBars();
                backDataDialog.setProgress(100, priceBars.size() + " bars", "Done");
                msg = priceBars.size() + " bars have been downloaded successfully.";
                eventReport.report(msg);
    ...
    public void cancel() {
        try {
            isCancelled = true;
            if (requestID != 0) {
                eventReport.report("Cancelling OpenTick historical request " + requestID);
    ...
    private void writeDescriptior() {
        writer.println("# This dataset is generated by JSystemTrader. Data source: OpenTick");
    ...
    private void writeBars() {
        ...
        for (PriceBar priceBar : priceBars) {
            ...
            if (barsWritten % 100 == 0) {
                ...
                backDataDialog.setProgress((int) progress, "", "Writing downloaded data to file");
            }
        }
    }
}
