...
struct task_info {
    ...
    bool my_token_ready;
    //! False until context has been designated
    bool my_context_designated;
    //! Storage for Context Keys for atomic operations
    ContextKeyMap* key_map;
    //! Atomic readiness counter
    tbb::atomic<int>* ready_count;
    ...
    void reset() {
        ...
        //my_context_token = 0;
        //my_context_token_ready = false;
        my_context_designated = false;
        ready_count = NULL;
        key_map = NULL;
        is_valid = false;
    }
...
public:
    ...
    //! Empty buffer for context_ordered filters
    input_buffer () :
          array(NULL), array_size(0),
          low_token(0), high_token(0),
          is_ordered(true), is_bound(false) {
       grow(initial_buffer_size);
       __TBB_ASSERT( array, NULL );
    }
    ...
    void clear( filter* my_filter ) {
        ...
        for( size_type i=0; i<array_size; ++i, ++t ){
            ...
            if (temp.is_valid ) {
                ...
                temp.is_valid = false;
                //clear key
                if ( temp.my_context_designated ) {
                    //delete temp.*key_set.begin();
                    //if ( temp.is_atomic )
                    delete temp.ready_count; 
                    if ( temp.key_map ) {
                        delete temp.key_map;
                    }
                }
...
public:
    ...
    void put_task_info(task_info &where_to_put ) {
        ...
        where_to_put.my_context_designated = my_context_designated;
        where_to_put.key_map = key_map;
        where_to_put.ready_count = ready_count;
...
task* stage_task::execute() {
    ...
    if( my_at_start ) {
        ...
        // If context ordered filter, do not execute but designate context. 
        if ( my_filter->is_context_ordered() && !my_context_designated ) {
            //Designate context
            ContextKeySet key_set = my_filter->designate_context (my_object);
            if (key_set.size() == 0)
              key_set.insert("A");
            my_context_designated = true;
            if ( my_filter->put_key_set(key_set,*this) ) {
                //Notify next token
        ...
        my_object = (*my_filter)(my_object);
        if( my_filter->is_serial() && !my_filter->is_context_ordered() )
            my_filter->my_input_buffer->note_done(my_token, *this);
        // Execute Context ordered filter if context was designated earlier.
        else if ( my_filter->is_context_ordered() ) {
            my_filter->note_done_key_map(*this);
        }
    }
    ...
    if( my_filter ) {
        ...
        add_to_depth(1);
        if( my_filter->is_serial() ) {
            // Context Ordered filter
            if ( my_filter->is_context_ordered() ) {
                // If this is first instance of Context based filter, we need to queue tasks to ensure proper ordering
                if ( !my_context_designated ) {
                     // Queue before designating contexts to tasks
                     if ( my_filter->my_input_buffer->put_token(*this) ) {
                          my_filter = NULL;
                          return NULL;
                     }
                     // If this is next token, let it go through for recycling. designated context will be executed (see above code)
                }
                else if ( my_filter->put_key_map(*this) ) {
                    //Task deferred
                    my_filter = NULL;
                    return NULL;
                }
        ...
        // Need to clear context if we are the last item in a context
        if ( my_context_designated ) {
                //Need to check and clean key_map
                internal::ContextKeyMap::iterator iterator;
                for (iterator=key_map->begin();iterator!=key_map->end();iterator++) {
                    ContextKey key = (*iterator).first;
                    Token token = (*iterator).second;
                    my_pipeline.clean_context(key,token);
                }
                //delete ready count
                delete ready_count;
                delete key_map;
    ...
    for( filter* f = filter_list; f; f = f->next_filter_in_pipeline ) {
        if ((f->my_filter_mode & filter::version_mask) >= __TBB_PIPELINE_VERSION(4))
            if( internal::input_buffer* b = f->my_input_buffer )
                b->clear(f);
        // clear all contexts
        if ( f->is_context_ordered() ) {
            for ( internal::ContextTable::iterator i=f->context_table.begin(); i!=f->context_table.end(); ++i ) {
                if ( internal::input_buffer*b = i->second ) {
                    b->clear(f);
                    delete b;
                }
            }
            f->context_table.clear();
        }
    }
}
#endif
...
//! Context filter task deferral
template<typename StageTask>
bool filter::put_token ( ContextKey& key, StageTask& putter ) {
    internal::Token token;
    internal::ContextKeyMap::iterator iterator = putter.key_map->find(key);
    token = (*iterator).second;
    internal::ContextTable::accessor acc;
    internal::input_buffer* buffer;
    if (!context_table.find(acc, key)) {
        context_table.insert(acc,key);
        acc->second = new internal::input_buffer ();
    }
    buffer = acc->second;
    if ( token!=buffer->low_token ) {
        // Need to catch up
        if ( token-buffer->low_token>=buffer->array_size ) 
             buffer->grow( token-buffer->low_token+1 );
        ITT_NOTIFY( sync_releasing, this );
        putter.put_task_info(buffer->array[token&(buffer->array_size-1)]);
        return true;
    }
    return false;
}
...
template<typename StageTask>
void filter::note_done ( internal::Token token, ContextKey& key, StageTask& spawner ) {
    internal::task_info wakee;
    int orig = 0;
    wakee.reset();
    {
          internal::ContextTable::accessor a;
          context_table.find(a, key);
          internal::input_buffer* buffer = a->second;
          if ( token==buffer->low_token ) {
              // Wake the next task
              internal::task_info& item = buffer->array[++buffer->low_token & (buffer->array_size-1)];
              ITT_NOTIFY( sync_acquired, this );
              wakee = item;
              item.is_valid = false;
          }
          if ( wakee.is_valid ) {
              //This item is ready
              orig = wakee.ready_count->fetch_and_decrement();
              if ( orig == 1 ) {
                  //Atomic task is ready to proceed
                  spawner.spawn_stage_task(wakee);
              }
          }
    }
...
template<typename StageTask>
bool filter::put_key_set( ContextKeySet& keys, StageTask& putter ) {
    ContextKeySet::iterator iterator;
    //Get tokens here
    if ( !putter.key_map ) {
      //Allocate new key_map
      putter.key_map = new internal::ContextKeyMap();
      for (iterator=keys.begin();iterator!=keys.end();iterator++) {
          // Get token from global token table  
          internal::ContextTokenTable::accessor a;
          my_pipeline->context_token_table.insert (a, *iterator);
          internal::Token tkn = a->second++;
          putter.key_map->insert(make_pair(*iterator,tkn)); 
      }
    }
    return put_key_map(putter);
}

template<typename StageTask>
bool filter::put_key_map( StageTask& putter ) {

     //Reset ready_count
     if ( !putter.ready_count ) {
          putter.ready_count = new atomic<int> ();
     }
     putter.ready_count->fetch_and_store(putter.key_map->size());
     bool saved=false;
     bool proceed = false;
     internal::ContextKeyMap::iterator iterator;
     for (iterator=putter.key_map->begin();iterator!=putter.key_map->end();iterator++) {
      ContextKey key = (*iterator).first;
      if ( this->put_token(key, putter) ) {
          saved = true;
      } else {
          //This key is ready
          int orig = putter.ready_count->fetch_and_decrement();
          //If we decremented this to zero, we need to proceed with processing
          if ( orig == 1 ) {
            proceed = true;
          }
      }
...
template<typename StageTask>
void filter::note_done_key_map( StageTask& spawner ) {
    internal::ContextKeyMap::iterator iterator;
    for (iterator=spawner.key_map->begin();iterator!=spawner.key_map->end();iterator++) {
        ContextKey key = (*iterator).first;
        internal::Token token = (*iterator).second;
        this->note_done(token,key,spawner);
    }
}


void pipeline::clean_context(ContextKey& key, internal::Token context_token) {
    //Acquire global lock
    internal::ContextTokenTable::accessor a;
    if ( context_token_table.insert(a, key) ) { 
        if ( context_token==a->second-1 ) {
            //I am the last context, need to clear all context entries
            for ( filter* f = filter_list; f; f = f->next_filter_in_pipeline ) {
                if ( f->is_context_ordered() ) {
                    internal::ContextTable::accessor acc;
                    if ( f->context_table.insert(acc, key) ) {
                        // Delete input_buffer
                        delete acc->second;
                        f->context_table.erase(acc);
                    }
                }
            }
            //Delete global entry
            context_token_table.erase(a);
        }
    }
}
