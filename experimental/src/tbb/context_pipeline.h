...
typedef std::basic_string<char,std::char_traits<char>,tbb::tbb_allocator<char> > ContextKey;
typedef std::set<ContextKey> ContextKeySet;
...
typedef std::map<ContextKey, Token> ContextKeyMap;
...
//! Concurrent Hash Map used to store context input buffers in serial_with_context filter
typedef concurrent_hash_map<ContextKey, input_buffer*> ContextTable;
typedef concurrent_hash_map<ContextKey, Token> ContextTokenTable;
...
private:
    ...
    //! EximiuS: 6th bit distinguishes context ordered filter.
    static const unsigned char filter_is_context_ordered = 0x1<<6;
...
public:
    ...
    enum mode {
        ...
        //! EximiuS: process items one at a time in a given context
        serial_context_order = current_version | filter_is_serial | filter_is_context_ordered,
...
public:
    ...
    //! EximiuS: True if filter must receive context items in order.
    bool is_context_ordered() const {
        return ( my_filter_mode & filter_is_context_ordered )==filter_is_context_ordered;
    }
    ...
    //! EximiuS: Operate on an item from the input stream, and return context key for context filter. Application must override this for better performance.
    virtual ContextKeySet designate_context ( void* item ) {
      ContextKeySet keys;
      ContextKey default_key = ContextKey (1, 'A');
      /*default_key->key_length = 1;
      default_key->key = new unsigned char(0xFF);*/
      keys.insert(default_key);
      return keys;
    }

    template<typename StageTask> bool put_token ( ContextKey &key, StageTask &putter );
    template<typename StageTask> void note_done ( internal::Token token, ContextKey& key, StageTask& spawner );
    template<typename StageTask> bool put_key_set ( ContextKeySet& keys, StageTask &putter );
    template<typename StageTask> bool put_key_map ( StageTask &putter );
    template<typename StageTask> void note_done_key_map ( StageTask& spawner );
    //! Destroy filter.
...
private:
    ...
    //! ContextTable for context based filter
    internal::ContextTable context_table;
};
...
private:
    ...
    //! EximiuS: ContextTokenTable for distributing global tokens to context items
    internal::ContextTokenTable context_token_table;
    ...
    //! EximiuS: Clean Contexts
    void clean_context(ContextKey& key, internal::Token context_token);
